...
function getWinChance(userId) {
    ...
    if (streak <= 5) return 0.45;
    if (streak <= 8) return 0.30;
    return 0.15;
}
...
// FIX: Bỏ client.on("debug") vì nó sinh ra hàng nghìn log mỗi giây làm chậm bot
// client.on("debug", console.log); ← ĐÃ XÓA
client.on("warn", console.log);

client.once("ready", () => {
    console.log("✅ BOT ONLINE:", client.user.username);

    // FIX: Tăng interval AOV lên 60 giây (thay vì 10s) để tránh spam API & rate limit
    setInterval(() => {
...
`
    ...
    const gradientColors = ["#FFFFFF", "#D1E1EC", "#A2C2D9", "#74A4C5", "#4585B1", "#0B3C5D", "#07263b"];
...
client.on("messageCreate", async (message) => {
    ...
    if (selected.has(message.channel.id)) {
        const ticketData = selected.get(message.channel.id);
        // FIX: Kiểm tra ticketData là object (ticket) hay string (stage selection)
        if (ticketData && typeof ticketData === "object" && ticketData.userId) {
...
client.on("interactionCreate", async (interaction) => {
    try {
                ...
                if (subcommand === "create") {
                    try {
                        ...
                        const loadingEmbed = new EmbedBuilder()
                            .setColor("#f0a500")
                            .setAuthor({
                                name: `${interaction.user.username} yêu cầu sao lưu`,
                        ...
                        // FIX: Lưu file MỚI trước, xóa file CŨ sau (an toàn hơn)
                        const newFilePath = path.join(backupPath, `${backupData.id}.json`);
                        fs.writeFileSync(newFilePath, JSON.stringify(backupData, null, 2), "utf8");
                        console.log(`[BACKUP] 📝 Đã lưu file mới: ${newFilePath}`);

                        // Xóa toàn bộ file cũ (giữ lại chỉ file vừa tạo)
                        let deletedCount = 0;
                        ...
                        for (const file of files) {
                            if (file.endsWith(".json") && file !== `${backupData.id}.json`) {
                                try {
                                    ...
                                    deletedCount++;
                                    console.log(`[BACKUP] 🗑️ Đã xóa file cũ: ${file}`);
                                } catch (err) {
                                    console.log("⚠️ Lỗi xóa file:", file, err.message);
                                }
                            }
                        }
                        ...
                        const successEmbed = new EmbedBuilder()
                            .setColor("#00ff88")
                            .setAuthor({
                                name: `${client.user.username} · Hệ thống Backup`,
                ...
                try {
                    ...
                    const embed = new EmbedBuilder()
                        ...
                        .addFields(
                            ...
                            { name: "💰 Số dư hiện tại", value: `**${getCoins(userId).toLocaleString()} coin**`, inline: true },
                            {
        ...
        // ===== BUTTON INTERACTIONS =====
            ...
            // FIX: Kiểm tra tdx_ trước bc_ để tránh conflict
            if (interaction.customId.startsWith("tdx_")) {
                ...
                // FIX: addCoins winMoney = betAmount (vốn) + betAmount (lời) khi thắng
                if (userChoice === result) {
                    addCoins(userId, betAmount * 2); // hoàn vốn + tiền thắng = 2x
                    await interaction.update({ 
                        content: `✅ Kết quả là **${resultText}**. Bạn thắng **${betAmount.toLocaleString()} coin**!`, 
                        embeds: [], 
                        components: [] 
                    });
                } else {
            ...
            // FIX: bc_ button chỉ mở modal, không conflict với bc_bet_ modal
            ...
            // FIX: cf_ button — split đúng index
            if (interaction.customId.startsWith("cf_")) {
                // customId dạng: "cf_ngua_500" hoặc "cf_up_500"
                const parts = interaction.customId.split("_");
                const betSide = parts[1];  // "ngua" hoặc "up"
                ...
                if (win) {
                    const winAmount = Math.floor(money * 1.95);
                    addCoins(userId, winAmount);
                    await interaction.editReply({ 
                        content: `🎉 Kết quả là: **${resultText}**. Bạn thắng và nhận được **${winAmount.toLocaleString()}** coin!` 
                    });
                } else {
                    await interaction.editReply({ 
                        content: `💀 Kết quả là: **${resultText}**. Bạn đã mất **${money.toLocaleString()}** coin. Chúc may mắn lần sau!` 
                    });
                }
                return;
            }
            ...
            if (interaction.customId === "close_ticket") {
                ...
                // FIX: Dùng reply ephemeral rồi xóa channel sau delay (tránh followUp sau deferUpdate gây lỗi)
        ...
        // ===== SELECT MENU INTERACTIONS =====
            ...
            if (interaction.customId === "match_info") {
                // FIX: Thêm ephemeral để tránh spam public
        ...
        // ===== MODAL SUBMIT INTERACTIONS =====
            ...
            // FIX: bc_bet_ modal — lấy fields value TRƯỚC, defer NGAY sau đó
            if (interaction.customId.startsWith("bc_bet_")) {
                const userId = interaction.user.id;

                try {
                    // Lấy dữ liệu từ modal TRƯỚC KHI defer (vì sau defer không lấy được nữa)
                    ...
                    // Defer ngay sau khi lấy xong fields
                    await interaction.deferReply();
                    ...
                    if (interaction.deferred || interaction.replied) {
                        return interaction.editReply({ content: "❌ Đã xảy ra lỗi hệ thống khi xử lý kết quả!" }).catch(() => {});
                    } else {
                        return interaction.reply({ content: "❌ Lỗi khởi tạo trò chơi!", ephemeral: true }).catch(() => {});
                    }
                }
            }

            // FIX: bet_ modal — lấy fields value TRƯỚC, defer NGAY sau đó
            if (interaction.customId.startsWith("bet_")) {
                try {
                    const userId = interaction.user.id;
                    const choice = interaction.customId.split("_")[1]; // "tai" hoặc "xiu"

                    // Lấy fields TRƯỚC KHI defer
                    const moneyInput = interaction.fields.getTextInputValue("money");
                    const money = parseInt(moneyInput);

                    // Defer ngay sau khi lấy xong
                    await interaction.deferReply();
                    ...
                    const winChance = getWinChance(userId);
                    const win = Math.random() < winChance;
                    updateStreak(userId, win);

                    // FIX: totalLabel phải so sánh với choice (tai/xiu), không phải giá trị xúc xắc
                    const diceResult = total >= 11 ? "tai" : "xiu";
                    const totalLabel = total >= 11 ? "TÀI" : "XỈU";
                    const playerWin = win ? (choice === diceResult) : (choice !== diceResult);
                    // Thực ra: win/lose được quyết định bởi getWinChance, không phải dice thật
                    // Để nhất quán: nếu win thì hiện dice tương ứng với choice người chơi
                    ...
                    if (win) {
                        ...
                        addCoins(userId, money + winMoney); // hoàn vốn + lời
        ...
        // FIX: Bỏ qua lỗi 10062 (Interaction hết hạn) — đây là lỗi bình thường, không cần thông báo
        if (err?.code === 10062 || err?.message?.includes("Unknown Interaction")) {
            console.warn("⚠️ Interaction đã hết hạn (10062), bỏ qua.");
            return;
        }

        // Bỏ qua lỗi 40060 (already acknowledged)
        if (err?.code === 40060 || err?.message?.includes("already been acknowledged")) {
            console.warn("⚠️ Interaction đã acknowledged (40060), bỏ qua.");
            return;
        }
        ...
        try {
            const errorMsg = { content: "❌ **Đã có lỗi xảy ra!**\nVui lòng thử lại sau hoặc liên hệ Admin.", ephemeral: true };
            if (interaction.deferred || interaction.replied) {
...
app.get("/staff-realtime", async (req, res) => {
    try {
        ...
        const roleIds = Object.values(ROLE_MAP).filter(Boolean); // FIX: Lọc undefined
